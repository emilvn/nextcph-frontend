...
const renderGridContent = (data: IOverviewData) => (
  <div className="grid grid-cols-4 gap-4 mb-4">
    <div className="bg-white p-4 border border-gray-200 rounded">
      <p className="text-gray-800">Total omsætning</p>
    ...
    <div className="bg-white p-4 border border-gray-200 rounded">
      <p className="text-gray-800">Gennemsnitlig daglig omsætning</p>
      <p className="text-lg font-bold">{data.averageDailyRevenue}</p>
    </div>
    <div className="bg-white p-4 border border-gray-200 rounded">
      <p className="text-gray-800">Gennemsnitlig daglig antal salg</p>
      <p className="text-lg font-bold">{data.averageDailySales.toFixed(2)} stk. </p>
...
const renderTableContent = (categories: IOverviewCategory[]) => (
  <table className="min-w-full">
    <thead>
      <tr>
        <th className="bg-gray-200 border border-gray-300 p-2">Kategori</th>
    ...
    <tbody>
      {categories.map((category: IOverviewCategory, index: number) => (
        <tr key={index}>
          <td className="border border-gray-300 p-2">{category.name}</td>
          <td className="border border-gray-300 p-2">{category.total}</td>
          <td className="border border-gray-300 p-2">{category.percentage}%</td>
        </tr>
      ))}
    </tbody>
  </table>
...
const renderOverviewContent = (data: IOverviewData) => (
  <div>
    {renderGridContent(data)}
    {data.categories && renderTableContent(data.categories)}
  </div>
);
    ...
    <div className="bg-gray-100 p-4">
      ...
      {!isLoading && (
        <div className="border border-gray-300 p-4">
          <h2 className="text-lg font-bold mb-4">Salgs data oversigt</h2>
          {Array.isArray(overviewData) ? (
            overviewData.map((data, index) => <div key={index}>{renderOverviewContent(data)}</div>)
          ) : (
            <div>{renderOverviewContent(overviewData as IOverviewData)}</div>
          )}
        </div>
      )}
    </div>
