...
function ProductRow(props: IProductRowProps) {
  return (
    <tr className="border-b-2">
      <td className="p-4 font-semibold md:text-3xl">{props.product.name}</td>
      <td className="md:text-3xl">{props.product.quantity}</td>
      <td className="md:text-xl">{formatPrice(props.product.price)}</td>
      <td>
        <button className="btn-white" onClick={props.increment}>
          <FaPlus className="text-xl inline-block" />
        </button>
      </td>
      <td>
        <button className="btn-white" onClick={props.decrement}>
            ...
            <ProductList>
              {props.products.map((product) => (
                <ProductRow
                  key={product.id}
                  product={product}
                  increment={() => props.incrementProduct(product)}
                  decrement={() => props.decrementProduct(product)}
                />
              ))}
            </ProductList>
          )}
          ...
          <div className="md:fixed max-md:right-0 md:right-20 flex flex-col justify-center gap-4 p-4 bg-next-white border border-next-blue h-80">
            <div className="w-80">
              ...
              <p className="text-2xl text-right">
                {props.products.reduce((a, product) => a + product.quantity, 0)}{" "}
                produkter
              </p>
            ...
            <SaleButtons submit={finishSale} reset={resetSale} />
          </div>
        )}
      </div>
    </>
